...
def _apply_chrome(win) -> "tk.Frame":
    """Delegate to the main module's _apply_panel_chrome for double-outline + shadow."""
    import sys
    gui = sys.modules.get("Snyk_Scanner_GUI") or sys.modules.get("__main__")
    fn = getattr(gui, "_apply_panel_chrome", None)
    if fn:
        return fn(win)
    # Fallback: plain accent border (original behaviour)
    T = _T()
...
class AppProfileEditor(tk.Toplevel):
    ...
    Usa exactamente el mismo patrón que ScannerApp._create_popup:
    tamaño 0.75×0.75 del monitor actual, centrado sobre master,
    overrideredirect + grab_set + _popup_follow_parent.
    """
    def __init__(self, master, store: AppInventoryStore,
        ...
        # ── Mismo sizing que _create_popup: 75% del monitor, centrado sobre master ──
        import sys as _sys
        _gui = _sys.modules.get("Snyk_Scanner_GUI") or _sys.modules.get("__main__")
        _monitor_wh  = getattr(_gui, "_monitor_wh",  None)
        _popup_follow = getattr(_gui, "_popup_follow_parent", None)
        master.update_idletasks()
        if _monitor_wh:
        ...
        if _popup_follow:
            _bind_ids.update(_popup_follow(self, master))

        # ── Chrome: double-outline + drop-shadow ──────────────────────────
        content = _apply_chrome(self)
        content.close = _close

        # ── Header: use master's _popup_hdr for consistent chrome ────────
        title_text = ("✏  Editar Perfil de Aplicación" if app else "📋  Perfil de Aplicación")
        self._app_editor_close = _close   # expose for external callers if needed
        master._popup_hdr(
            content, title_text,
        ...
        # ── Footer: use master's _popup_foot for consistent chrome ────────
