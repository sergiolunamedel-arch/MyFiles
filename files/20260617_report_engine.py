...
def _smart_truncate(text: str, limit: int = 320) -> str:
...
def _extract_remediation(help_text: str) -> str:
    ...
    for marker in _REMEDIATION_MARKERS:
        ...
        seg = seg.strip(" :\n-*")
        if seg:
            return _smart_truncate(seg, 320)
    ...
    if best:
        return _smart_truncate(best, 320)
    return ""
    ...
    for blk in runs:
        ...
        for result in (blk.get("results") or []):
            ...
            if fixes_list:
                ...
                if desc0: recommendation = _smart_truncate(desc0, 320)
            if not recommendation and help_text:
                recommendation = _extract_remediation(help_text)
            if not recommendation:
                recommendation = _smart_truncate(rule_props.get("recommendation", ""), 320)
  ...
  {% elif p.issues %}<div class="table-wrap"><table>
    ...
    <tr class="row" data-sev="{{ i.severity }}"
      ...
      <td>{% if i.url %}<a href="{{ i.url }}" target="_blank">{{ i.title }}</a>{% else %}{{ i.title }}{% endif %}
        ...
        {% if i.recommendation %}<div class="fix"><b>🛠 Fix:</b> {{ i.recommendation }}</div>{% endif %}</td>
