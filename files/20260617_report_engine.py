    ...
    for p in (data if isinstance(data, list) else [data]):
        ...
        for v in (p.get("vulnerabilities") or []):
            ...
            if is_upgradeable and upgrade_path:
                ...
                recommendation_sca = (v.get("description") or "")[:200] or "No automated fix — review manually"
    ...
    for blk in runs:
        ...
        for result in (blk.get("results") or []):
            ...
            if fixes_list:
                ...
                if desc0: recommendation = desc0[:400]
            if not recommendation and help_text:
                # Take first non-empty line of the help block
                for ln in help_text.splitlines():
                    ln = ln.strip()
                    if ln and not ln.startswith("#"):
                        recommendation = ln[:400]; break
            if not recommendation:
                recommendation = rule_props.get("recommendation", "")[:400]
...
{% for p in projects %}<details class="group" {% if p.issues %}open{% endif %}>
  ...
  {% elif p.issues %}<div class="table-wrap"><table>
    <thead><tr><th>Severity</th><th>Vulnerable Package</th><th>Installed ver.</th><th>Fixed in</th><th>Recommendation to fix</th><th>Title / Advisory</th><th>CVE</th><th>CWE</th><th>Dependency path</th><th>Transitive?</th></tr></thead><tbody>
    {% for i in p.issues %}
    ...
    <tr class="row" data-sev="{{ i.severity }}"
      data-text="{{ (i.package~' '~i.title~' '~i.cve~' '~i.cwe~' '~i.path~' '~(i.recommendation or ''))|lower }}">
      ...
      <td style="font-size:11px;max-width:220px">{% if i.recommendation %}<span style="color:var(--low);font-weight:600">{{ i.recommendation }}</span>{% else %}<span style="color:var(--mu)">—</span>{% endif %}</td>
...
{% for f in files %}<details class="group" open>
  ...
  <div class="table-wrap"><table>
    <thead><tr><th>Severity</th><th>Rule ID</th><th>Rule name</th><th>Line</th><th>CWE</th><th>OWASP category</th><th>Message</th><th>Recommendation to fix</th><th>Code snippet</th></tr></thead><tbody>
    ...
    <tr class="row" data-sev="{{ i.severity }}"
      data-text="{{ (i.rule~' '~i.title~' '~i.message~' '~i.cwe~' '~(i.recommendation or ''))|lower }}">
      <td><span class="sev {{ i.severity }}">{{ i.severity }}</span></td>
