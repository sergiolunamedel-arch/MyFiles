...
class AppProfileEditor(tk.Toplevel):
    ...
    def __init__(self, master, store: AppInventoryStore,
        ...
        # ── Mismo chrome que _create_popup: borde accent + bg ─────────────
        ...
        # ── Header — mismo estilo que _popup_hdr ──────────────────────────
        hdr_bg = T["panel_bg"]
        hdr = tk.Frame(content, bg=hdr_bg, padx=30, pady=16)
        hdr.pack(fill="x")
        row = tk.Frame(hdr, bg=hdr_bg); row.pack(fill="x")
        title_text = ("✏  Editar Perfil de Aplicación" if app else "📋  Perfil de Aplicación")
        tk.Label(row, text=title_text, font=(FUI, 15, "bold"),
                 bg=hdr_bg, fg=T["accent"]).pack(side="left")
        tk.Label(row, text="Complete los campos — solo el Nombre es obligatorio.",
                 font=(FUI, 11, "italic"), bg=hdr_bg, fg=T["muted"]).pack(side="left", padx=(14, 0))
        xbtn = tk.Label(row, text="✕", font=(FUI, 13, "bold"),
                        bg=hdr_bg, fg=T["muted"], cursor="hand2", padx=6)
