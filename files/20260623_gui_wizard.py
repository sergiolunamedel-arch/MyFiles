...
class _WizardWindow:
    ...
    def _build_step_static_paths(self, parent: tk.Frame) -> None:
        ok, msg = self._app._wiz_check_static_paths()
        self._body_text(parent,
            "Agregue las carpetas o archivos de código que serán analizados estáticamente.\n\n"
            "SCA busca vulnerabilidades en dependencias de código abierto.  "
            "SAST analiza el código fuente en busca de debilidades de seguridad.  "
            "Secrets detecta credenciales y tokens expuestos.\n\n"
            "Puede agregar múltiples carpetas o proyectos; el motor los recorre en orden.")
        ...
        def _sync_to_app():
            ...
            if rp and tv:
                tv.set(rp[-1])
            # Refresh status row
            ok2, msg2 = self._app._wiz_check_static_paths()
            sf = getattr(self, "_wiz_static_status_frame", None)
            if sf and sf.winfo_exists():
                for w in sf.winfo_children():
                    w.destroy()
                self._status_row(sf, ok2, msg2)
        ...
        _hint = tk.Label(
            sec,
            text="Tip: agregue la raíz de su repositorio para cubrir todos los proyectos.  "
                 "Para mono-repos puede agregar múltiples sub-carpetas.",
            font=(_FUI, 9), bg=T["panel_bg"], fg=T["muted"],
            justify="left", anchor="w",
        )
        _hint.pack(fill="x", pady=(0, 4))
        def _rh(e, l=_hint): l.config(wraplength=max(200, e.width - 4))
        _hint.bind("<Configure>", _rh)
        ...
        _omit_note = tk.Label(
            sec,
            text="¿Sin código local? Haga clic en Omitir — se desactivarán SCA, SAST y Secrets.",
            font=(_FUI, 9, "italic"), bg=T["panel_bg"], fg=T["muted"],
            justify="left", anchor="w",
        )
        _omit_note.pack(fill="x")
        def _ron(e, l=_omit_note): l.config(wraplength=max(200, e.width - 4))
        _omit_note.bind("<Configure>", _ron)

        # Status row (kept as instance attr so _sync_to_app can refresh it)
        self._wiz_static_status_frame = self._section(parent, "Estado")
        self._status_row(self._wiz_static_status_frame, ok, msg)
