...
class _WizardWindow:
    ...
    def _build_step_app(self, parent: tk.Frame) -> None:
        ...
        if ok:
            ...
            # Botón "Cambiar" — visible sólo cuando ya hay app activa
            btn_row = tk.Frame(sec_active, bg=T["panel_bg"])
            btn_row.pack(anchor="w", pady=(8, 0))
            self._app._btn(btn_row, "🔄  Cambiar aplicación",
                           lambda: self._toggle_app_picker(parent), "flat").pack(side="left")

        # ── Sección: selector / creador de app ───────────────────────────
        sec2 = self._section(parent, "Seleccionar o crear perfil de aplicación")

        # Toolbar: Nueva app + Cargar
        toolbar = tk.Frame(sec2, bg=T["panel_bg"])
        toolbar.pack(fill="x", pady=(0, 6))
        self._app._btn(toolbar, "➕  Nueva aplicación",
                       lambda: self._wiz_new_app(parent), "accent").pack(side="left", padx=(0, 8))
        self._wiz_load_btn = self._app._btn(toolbar, "▶  Cargar seleccionada",
                                            self._wiz_load_selected, "flat")
        self._wiz_load_btn.pack(side="left")
        self._wiz_load_btn.config(state="disabled")
        ...
        list_outer = tk.Frame(sec2, bg=T["panel_bg"],
        ...
        if not apps:
            tk.Label(list_outer,
                     text="No hay aplicaciones registradas — cree una con ➕ Nueva aplicación.",
                     font=(_FUI, 9), bg=T["panel_bg"], fg=T["muted"],
                     anchor="w", padx=8, pady=8).pack(fill="x")
            self._wiz_app_listbox = None
            self._wiz_apps        = []
        else:
            self._wiz_apps = apps
            # Listbox con scrollbar
            lb_wrap = tk.Frame(list_outer, bg=T["panel_bg"])
            lb_wrap.pack(fill="x")
            sb = ttk.Scrollbar(lb_wrap, orient="vertical")
            self._wiz_app_listbox = tk.Listbox(
                lb_wrap,
                height=min(6, len(apps)),
                font=(_FUI, 10),
                bg=T["panel_bg"],
                fg=T["text"],
            ...
            sb.config(command=self._wiz_app_listbox.yview)
            self._wiz_app_listbox.pack(side="left", fill="x", expand=True)
            sb.pack(side="right", fill="y")

            active_app = getattr(self._app, "_active_app", None)
            active_id  = active_app.get("id") if active_app else None
            for i, a in enumerate(apps):
                tier   = a.get("risk_tier", "Ninguno")
                c, h   = a.get("vuln_critical", 0), a.get("vuln_high", 0)
                m, lo  = a.get("vuln_medium", 0),   a.get("vuln_low", 0)
                vulns  = f"  C:{c} A:{h} M:{m} B:{lo}" if (c or h or m or lo) else ""
                marker = "  ✓ activa" if a.get("id") == active_id else ""
                self._wiz_app_listbox.insert(
                    "end",
                    f"  {a['name']}  ·  {a.get('criticality', '')}  ·  Riesgo: {tier}{vulns}{marker}",
                )
                if a.get("id") == active_id:
                    self._wiz_app_listbox.selection_set(i)
                    self._wiz_app_listbox.see(i)

            def _on_lb_select(_evt=None):
                sel = self._wiz_app_listbox.curselection()
                self._wiz_load_btn.config(
                    state="normal" if sel else "disabled")
                self._wiz_preview_app(sec2, sel)

            self._wiz_app_listbox.bind("<<ListboxSelect>>", _on_lb_select)
            self._wiz_app_listbox.bind("<Double-1>",        lambda _: self._wiz_load_selected())

            # Habilitar botón si ya hay algo seleccionado (app activa)
            if self._wiz_app_listbox.curselection():
                self._wiz_load_btn.config(state="normal")

        # Mini preview de la app resaltada
        self._wiz_preview_frame = tk.Frame(sec2, bg=T["panel_bg"])
        self._wiz_preview_frame.pack(fill="x", pady=(6, 0))

    # ── Helpers del selector de app dentro del wizard ─────────────────────

    def _wiz_preview_app(self, sec2: tk.Frame, sel: tuple) -> None:
        """Muestra nombre/tipo/URL de la app seleccionada en el listbox."""
        for w in self._wiz_preview_frame.winfo_children():
            w.destroy()
        if not sel:
            return
        apps = getattr(self, "_wiz_apps", [])
        if not apps or sel[0] >= len(apps):
            return
        app = apps[sel[0]]
        for lbl, val in [
            ("Tipo",    app.get("app_type",    "—")),
            ("Entorno", app.get("environment", "—")),
            ("URL DAST",app.get("dast_url",    "—") or "—"),
        ]:
            r = tk.Frame(self._wiz_preview_frame, bg=T["panel_bg"])
            r.pack(fill="x", pady=1)
            tk.Label(r, text=f"{lbl}:", font=(_FUI, 9),
                     bg=T["panel_bg"], fg=T["muted"],
                     width=10, anchor="w").pack(side="left")
            tk.Label(r, text=val, font=(_FUI, 9),
                     bg=T["panel_bg"], fg=T["text"],
                     anchor="w").pack(side="left")

    def _wiz_load_selected(self) -> None:
        """Carga la app seleccionada en el listbox como perfil activo."""
        lb = getattr(self, "_wiz_app_listbox", None)
        if lb is None:
            return
        sel = lb.curselection()
        if not sel:
        ...
        app = apps[sel[0]]
        try:
            self._app._load_app_profile(app, silent=True)
        except Exception:
            pass
        # Re-renderizar el paso para reflejar la nueva app activa
        self._render_step(self._step)

    def _wiz_new_app(self, parent: tk.Frame) -> None:
        """Abre AppProfileEditor como modal del wizard y recarga el paso al guardar."""
        try:
            ...
            if store is None:
                ...
                return
            ed = AppProfileEditor(self._win, store)
            self._win.wait_window(ed)
            if ed.result:
                # Auto-cargar la nueva app como activa
                try:
    ...
    def _toggle_app_picker(self, parent: tk.Frame) -> None:
        """Re-renderiza el paso para que el usuario pueda elegir otra app."""
        self._render_step(self._step)
