...
# --- Multi-environment support (Development / QA / Producción) -----------------------------
#
# Everything above this block (BASE_URL, SYS_ID, CANCEL_*, CREDENTIALS_FILE) remains the single
# "default" environment used by the CLI when no --service/env flags are given, so existing CLI
# usage (env vars, `--remember`, etc.) is completely unaffected. The GUI drives three independent
# `Environment` instances instead of these module-level globals, each with its own connection
# settings and its own credentials file (Development deliberately reuses CREDENTIALS_FILE above so
# credentials already saved via the CLI or the pre-tabs GUI keep working without re-entry).
...
class Environment:
    ...
    base_url: str = ""
    sys_id: str = ""
    cancel_table: str = "sc_request"
    cancel_state_field: str = "state"
    cancel_state_value: str = "Closed Cancelled"
    credentials_file: str = ""
    is_production: bool = False


def _environment_config_file(env_key: str) -> str:
    return os.path.join(APP_DATA_DIR, f"environment_{env_key}.json")


def _default_environment(env_key: str) -> Environment:
    if env_key == "dev":
        return Environment(
            key="dev", label="Development",
            base_url=BASE_URL, sys_id=SYS_ID,
            cancel_table=CANCEL_TABLE, cancel_state_field=CANCEL_STATE_FIELD,
            cancel_state_value=CANCEL_STATE_VALUE,
    ...
    if env_key == "qa":
        return Environment(
            key="qa", label="QA",
            base_url="", sys_id="",
            credentials_file=os.path.join(APP_DATA_DIR, "credentials_qa.json"),
        )
    if env_key == "prod":
        return Environment(
            key="prod", label="Producción",
            base_url="", sys_id="",
            credentials_file=os.path.join(APP_DATA_DIR, "credentials_prod.json"),
            is_production=True,
        )
    raise ValueError(f"Ambiente desconocido: {env_key}")


ENVIRONMENT_KEYS = ("dev", "qa", "prod")


def load_environment(env_key: str) -> Environment:
    """Loads an Environment, seeded with defaults (real values for 'dev', blank for 'qa'/'prod'
    since those instance URLs/sys_ids aren't known here) and overlaid with whatever was saved
    to disk via save_environment() — e.g. after the user fills in the QA/Prod instance URL and
    catalog sys_id in the GUI once."""
    env = _default_environment(env_key)
    path = _environment_config_file(env_key)
...
def save_environment(env: Environment) -> None:
    path = _environment_config_file(env.key)
    with open(path, "w", encoding="utf-8") as f:
        json.dump(asdict(env), f, indent=2, ensure_ascii=False)
    try:
        os.chmod(path, 0o600)
    except OSError:
        pass
...
SERVICES = {
    ...
    "servidor": {
        "field": "servidor_tenable",
        "extra": {
            "direcci_nes_ip_o_hostnames": "10.0.0.1;10.0.0.2",
        },
    },
}
...
def load_saved_credentials(credentials_file: Optional[str] = None) -> dict:
...
def save_credentials(user: str = "", password: str = "", token: str = "",
...
def clear_saved_credentials(credentials_file: Optional[str] = None) -> None:
    path = credentials_file or CREDENTIALS_FILE
    try:
        os.remove(path)
...
def send_request(scenario_name: str, payload: dict, auth, extra_headers: dict, dry_run: bool,
                  log=print, *, base_url: Optional[str] = None, sys_id: Optional[str] = None) -> dict:
    base_url = base_url or BASE_URL
    sys_id = sys_id or SYS_ID
    order_url = f"{base_url}/api/sn_sc/servicecatalog/items/{sys_id}/order_now"
...
def cancel_request(sys_id: str, auth, extra_headers: dict, log=print, *,
                    base_url: Optional[str] = None, cancel_table: Optional[str] = None,
                    cancel_state_field: Optional[str] = None,
                    cancel_state_value: Optional[str] = None) -> dict:
    if not sys_id:
        return {"cancelled": False, "cleanup_error": "sin sys_id (no se puede cancelar)"}

    base_url = base_url or BASE_URL
    cancel_table = cancel_table or CANCEL_TABLE
    cancel_state_field = cancel_state_field or CANCEL_STATE_FIELD
    cancel_state_value = cancel_state_value or CANCEL_STATE_VALUE
...
def get_audit_history(table: str, sys_id: str, auth, extra_headers: dict, log=print, *,
...
def list_variables(auth, extra_headers: dict, mandatory_only: bool = True, log=print, *,
                    base_url: Optional[str] = None, sys_id: Optional[str] = None) -> list[dict]:
    base_url = base_url or BASE_URL
    sys_id = sys_id or SYS_ID
...
def diagnose_missing_variables(payload: dict, auth, extra_headers: dict, log=print, *,
                                base_url: Optional[str] = None, sys_id: Optional[str] = None) -> list[str]:
...
def get_variable_detail(auth, extra_headers: dict, name: str, log=print, *,
                         base_url: Optional[str] = None, sys_id: Optional[str] = None) -> dict:
    base_url = base_url or BASE_URL
    sys_id = sys_id or SYS_ID
    url = (f"{base_url}/api/now/table/item_option_new"
...
def list_recent_tickets(auth, extra_headers: dict, limit: int = 25, log=print, *,
                         base_url: Optional[str] = None, sys_id: Optional[str] = None) -> list[dict]:
    base_url = base_url or BASE_URL
    sys_id = sys_id or SYS_ID
...
def cleanup_results(results: list[dict], auth, extra_headers: dict, log=print, *,
                     base_url: Optional[str] = None, cancel_table: Optional[str] = None,
                     cancel_state_field: Optional[str] = None,
                     cancel_state_value: Optional[str] = None) -> None:
    for row in results:
        if row.get("status") in (200, 201) and row.get("sys_id"):
            cleanup = cancel_request(row["sys_id"], auth, extra_headers, log=log,
                                      base_url=base_url, cancel_table=cancel_table,
                                      cancel_state_field=cancel_state_field,
                                      cancel_state_value=cancel_state_value)
